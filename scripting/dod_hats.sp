...
new	hats_count,                       // Amount of available hats
...
public Event_PlayerHurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	// On death event hats wont disappear, but this one is works fine.
...
public Action:Command_Hat(client, args)
{
	if (GetConVarBool(dodhats_enable) && IsValidClient(client))
	{
		...
		if (hat_flag && !(flagc & ADMFLAG_ROOT))
		{
...
public Action:Command_DisableHat(client, args)
{
	if (GetConVarBool(dodhats_enable) && IsValidClient(client))
	{
		hats_disabled[client] = !hats_disabled[client];

		// Remove hat on disabling
...
LoadConfig()
{
	...
	if (!hats_count) SetFailState("No models wtf?!");
...
ShowPlayerList(client)
{
	...
	SetMenuTitle(menu, hats_adminmenu[client] ? title1 : title2);
...
RemoveHat(client)
{
	...
	// Properly remove hat from world
	if (IsValidEntRef(entity))
...
RandomHat()
{
	// Creates a hat with random index (i.e -1) on all players
	for (new i = 1; i <= MaxClients; i++)
...
LoadCookies(client)
{
	...
	hat_save[client] = (cookie[0] ? StringToInt(cookie) : 0);
}
